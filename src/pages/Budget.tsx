        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8 pb-10">
            ...
            <div>
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-2 text-slate-500 font-medium mb-2">
                    ...
                    <span>ระบบงบประมาณและพัสดุ</span>
                </motion.div>
                <motion.h1 className="text-4xl md:text-5xl font-bold text-slate-900 tracking-tight" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                    งบประมาณ<span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-violet-600">และพัสดุ</span>
                </motion.h1>
            </div>
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                    { icon: DollarSign, label: 'งบประมาณคงเหลือ', value: '฿456,700', sub: 'จากทั้งหมด ฿1,000,000', gradient: 'from-emerald-500 to-teal-500', shadow: 'shadow-emerald-200' },
                    { icon: TrendingDown, label: 'รายจ่ายเดือนนี้', value: '฿43,300', sub: '+12.5% จากเดือนที่แล้ว', gradient: 'from-blue-500 to-indigo-500', shadow: 'shadow-blue-200' },
                    { icon: Receipt, label: 'รายการรออนุมัติ', value: '3', sub: 'คำขอเบิกจ่าย', gradient: 'from-amber-500 to-orange-500', shadow: 'shadow-amber-200' },
                ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <motion.div variants={itemVariants} className="lg:col-span-2 bg-white/60 backdrop-blur-xl border border-white/60 rounded-3xl p-6 shadow-sm">
                    <h3 className="text-lg font-bold text-slate-800 mb-5 flex items-center gap-2">
                        <PieChart className="w-5 h-5 text-purple-500" /> สัดส่วนรายจ่าย
                    </h3>
                    <div className="space-y-4">
                        {[
                            { label: 'ครุภัณฑ์คอมพิวเตอร์', percent: 45, color: 'bg-blue-500' },
                            { label: 'วัสดุสำนักงาน', percent: 20, color: 'bg-emerald-500' },
                            { label: 'ค่าจ้างบริการ', percent: 15, color: 'bg-orange-500' },
                            { label: 'อื่นๆ', percent: 20, color: 'bg-slate-400' },
                        ].map((item, idx) => (
                            <div key={idx}>
                    ...
                    <div className="mt-6 pt-4 border-t border-slate-100">
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-slate-500">งบประมาณใช้ไป</span>
                ...
                <motion.div variants={itemVariants} className="lg:col-span-3 bg-white/60 backdrop-blur-xl border border-white/60 rounded-3xl p-6 shadow-sm">
                    <div className="flex justify-between items-center mb-5">
                        <div>
                            ...
                            <p className="text-sm text-slate-500">ประวัติการเบิกจ่ายและการอนุมัติ</p>
                        </div>
                        <Button className="rounded-xl bg-purple-600 hover:bg-purple-700 shadow-lg shadow-purple-200">
                            <Plus className="w-4 h-4 mr-2" /> ขอเบิกงบ
                        </Button>
                    </div>
                    <div className="space-y-2">
                        {transactions.map((t) => (
                            <motion.div key={t.id} whileHover={{ x: 4 }} className="flex items-center justify-between p-4 rounded-2xl hover:bg-white border border-transparent hover:border-slate-100 hover:shadow-sm transition-all">
                                <div className="flex items-center gap-4">
                                    <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${t.type === 'expense' ? 'bg-red-50 text-red-500' : 'bg-emerald-50 text-emerald-500'}`}>
                                        {t.type === 'expense' ? <TrendingDown className="w-5 h-5" /> : <TrendingUp className="w-5 h-5" />}
                                    </div>
                                    <div>
                                        <h4 className="font-semibold text-slate-800">{t.description}</h4>
                                        <p className="text-sm text-slate-400">{t.category} • {t.date}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    ...
                                    <Badge variant="outline" className={`text-xs mt-1 ${t.status === 'approved' ? 'border-emerald-200 text-emerald-600 bg-emerald-50' : 'border-amber-200 text-amber-600 bg-amber-50'}`}>
                                        {t.status === 'approved' ? 'อนุมัติแล้ว' : 'รอตรวจสอบ'}
                                    </Badge>
                                </div>
                            </motion.div>
                        ))}
