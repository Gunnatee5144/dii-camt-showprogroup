        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8 pb-10">
            ...
            <div>
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-2 text-slate-500 font-medium mb-2">
                    ...
                    <span>ระบบติดตามภาระงาน</span>
                </motion.div>
                <motion.h1 className="text-4xl md:text-5xl font-bold text-slate-900 tracking-tight" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                    ติดตาม<span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-violet-600">ภาระงานอาจารย์</span>
                </motion.h1>
            </div>
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    { icon: Users, label: 'อาจารย์ทั้งหมด', value: String(lecturerData.length), gradient: 'from-blue-500 to-indigo-500', shadow: 'shadow-blue-200' },
                    { icon: Clock, label: 'ภาระงานเฉลี่ย', value: `${avgWorkload} ชม.`, gradient: 'from-purple-500 to-violet-500', shadow: 'shadow-purple-200' },
                    { icon: AlertTriangle, label: 'เกินเกณฑ์', value: String(overloaded), gradient: 'from-red-500 to-rose-500', shadow: 'shadow-red-200' },
                    { icon: TrendingUp, label: 'ปกติ', value: String(lecturerData.length - overloaded), gradient: 'from-emerald-500 to-teal-500', shadow: 'shadow-emerald-200' },
                ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <motion.div variants={itemVariants} className="lg:col-span-2 bg-white/60 backdrop-blur-xl border border-white/60 rounded-3xl p-6 shadow-sm h-fit space-y-6">
                    <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                        <BookOpen className="w-5 h-5 text-purple-500" /> สถานะภาพรวม
                    </h3>
                    <div className="space-y-5">
                        <div>
                            <p className="text-sm text-slate-500 mb-1">ภาระงานสอนเฉลี่ย</p>
                            <div className="flex items-end gap-2">
                                ...
                                <span className="text-slate-400 mb-1.5">ชม./สัปดาห์</span>
                            </div>
                        </div>
                        ...
                        <div className="p-4 rounded-2xl bg-red-50 border border-red-100">
                            <div className="flex items-center gap-2 text-red-600 mb-1">
                                ...
                                <span className="font-bold">อาจารย์ที่มีภาระงานเกินเกณฑ์</span>
                            </div>
                            <span className="text-3xl font-bold text-red-600">{overloaded} ท่าน</span>
                            <p className="text-sm text-red-400 mt-1">เกณฑ์: มากกว่า 15 ชม./สัปดาห์</p>
                        </div>
                    </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="lg:col-span-3 bg-white/60 backdrop-blur-xl border border-white/60 rounded-3xl p-6 shadow-sm">
                    <div className="mb-5">
                        <h3 className="text-lg font-bold text-slate-800">รายละเอียดรายบุคคล</h3>
                        <p className="text-sm text-slate-500">ภาระงานสอนของอาจารย์แต่ละท่าน</p>
                    </div>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between mb-1.5">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className={`text-sm font-bold ${isOver ? 'text-red-500' : 'text-slate-600'}`}>{lecturer.workload} ชม.</span>
                                                {isOver && <Badge className="bg-red-50 text-red-500 text-[10px] border-red-200">เกินเกณฑ์</Badge>}
                                            </div>
                                        </div>
